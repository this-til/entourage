...
// user must send the bloc code for run directly after this macro :
...
uint8_t inline __incAtomic(void) {
    ++SCoopInstanceNickName.Atomic;
    return 1;
}
...
void inline __SCoopUnprotect(uint8_t** __s) {
    uint8_t* staticFlag = *__s;
    *staticFlag = 0;
};
...
uint8_t inline __SCoopNoInterrupts(void) {
    noInterrupts();
    return 1;
}
